    ...
    public static void main(String[] args) throws FileNotFoundException {        
        ...
        while (reader.hasNextLine()) {
            ...
            if (!(str.length() == 0)) {     //If no blank lines detected, then continue
            	String [] strSplit = str.trim().split("(?<=\\+\\+|\\-\\-|\\=\\=|\\<\\=|\\>\\=)|(?=\\+\\+|\\-\\-|\\=\\=|\\<\\=|\\>\\=)");     //Regex for splitting the code into lexemes
                for (int i = 0; i < strSplit.length; i++) {
                	if (strSplit[i].equals("++")) {
                		tokens.add(strSplit[i]);
                		strSplit[i]="";
                	}
                	else {
                		tempStr = tempStr+strSplit[i];
                	}
                }
                String[] strSplit2 = tempStr.trim().split("(?<=\\+|\\-|\\*|\\/|\\<|\\>|\\=|\\(|\\{|\\)|\\}|\\;)|(?=\\+|\\-|\\*|\\/|\\<|\\>|\\=|\\(|\\{|\\)|\\}|\\;)");
                for(String s:strSplit2) {
                	tokens.add(s);
                }
                List <String> list = Arrays.asList(strSplit2);       //Save the lexeme array into a Container List
